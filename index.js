	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Ola😏❤️  @${num.split('@')[0]}\nMembro novo paga croquete pros adm😅 *${mdata.subject}*`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mencionado Jid": [num]}})
		...
		try {
			...
			switch(command) {
				...
				case 'meme':
					...
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQzGjaoxNyDxpQr3-zwqOqOWjMgkgWsPRzOxw&usqp=CAU`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '.......'})
					break
                case 'loli':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSsBAmxbHmxsa3OAK2CSZ0lT3OiLdUj9Gu0VQ&usqp=CAU`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '(sfd 😳)'})
					break

                case '(bv)':
		            if (args.length < 1) return reply ('Ola bem vindo membro novo ❤️😏')
		            break

                case 'dono':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTRLiFCi9huGyYnAjGIj159dU3aSvjYWF6EDg&usqp=CAU`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*NOME* ELPABLO\n*WPP,* wa.me/+553299362937\n*INSTAGRAM:* Pablo_Marques_077'})
					break
				case 'porno':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS88ASQcISPfRJy6BSRlIycM40jurPKvegGig&usqp=CAU`)buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSNj6XnNncwSMwbwNDzvyqn-FfuSctpwTnhtA&usqp=CAU`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Eu sei que vc quer 😳'})
					break
	            case 'Bom dia':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSdTylpp2o0PjaIY-E-eVlMDX9MnA4S3K32Uw&usqp=CAU`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Bom dia seus gostosos'})
					break
				/*case 'memeindo':
					memein = await kagApi.memeindo()
				...
				case 'url2img':
					...
					if (!tipelist.includes(args[0])) return reply('Desktop | tablet | tipo 'celular)
				...
				case 'tsticker':
					...
					break
				case 'tagall':
					if (! isGroup) resposta de retorno (mess.only.group)
